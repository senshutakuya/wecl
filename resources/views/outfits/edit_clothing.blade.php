...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        <x-app-layout>
            ...
            <div class='posts'>
                <div class='post'>
                    <p class='content_title'>服を編集してね</p>
                </div>
            </div>
            <div class="main">
                ...
                <form action="{{ route('update', ['post' => $outfit->id]) }}" method ="post" enctype="multipart/form-data">
                    ...
                    <label for="frontCameraCapture">
                        ...
                        <input type="file" name="front_upfile_camera" class="cameraCapture" accept="image/*"  />
                    ...
                    <label for="backCameraCapture">
                        ...
                        <input type="file" name="back_upfile_camera" class="cameraCapture" accept="image/*"  />
                   ...
                   <div class="category">
                        ...
                        <select id="categorySelect" name="post[category_id]">
                            @foreach ($tops as $as_tops)
                                <option id="tops" value="{{ json_encode($as_tops) }}" {{ $outfit->tops_id == $as_tops->id ? 'selected' : '' }}>{{ $as_tops->tops }}</option>
                            @endforeach
                            @foreach ($botms as $as_botms)
                                <option id="botms" value="{{ json_encode($as_botms) }}" {{ $outfit->botms_id == $as_botms->id ? 'selected' : '' }}>{{ $as_botms->botms }}</option>
                            @endforeach
                            @foreach ($dores as $as_dores)
                                <option id="dores" value="{{ json_encode($as_dores) }}" {{ $outfit->dores_id == $as_dores->id ? 'selected' : '' }}>{{ $as_dores->dores }}</option>
                            @endforeach
                            @foreach ($outerwares as $as_outerware)
                                <option id="outerware" value="{{ json_encode($as_outerware) }}" {{ $outfit->outerware_id == $as_outerware->id ? 'selected' : '' }}>{{ $as_outerware->outerware }}</option>
                            @endforeach
                            @foreach ($accessories as $as_accessory)
                                <option id="accessory" value="{{ json_encode($as_accessory) }}" {{ $outfit->accessory_id == $as_accessory->id ? 'selected' : '' }}>{{ $as_accessory->accessory }}</option>
                            @endforeach
                            @foreach ($shoes as $as_shoes)
                                <option id="shoes" value="{{ json_encode($as_shoes) }}" {{ $outfit->shoes_id == $as_shoes->id ? 'selected' : '' }}>{{ $as_shoes->shoes }}</option>
                            @endforeach
                            @foreach ($overlaps as $as_overlap)
                                <option id="overlap" value="{{ json_encode($as_overlap) }}" {{ $outfit->overlap_id == $as_overlap->id ? 'selected' : '' }}>{{ $as_overlap->overlap }}</option>
                            @endforeach
                        </select>
                    </div>
                    <br>
                        ...
                        <select id="colorSelect" name="post[color_id]">
                            @foreach ($colors as $color)
                                @if (!empty($color->co_one_value))
                                    <option id ="co_one" value="{{ $color->id }}" @if ($outfit->color_id == $color->id) selected @endif>
                                        {{ $color->co_one_value }}
                                    </option>
                                @endif
                                @if (!empty($color->co_two_value))
                                    <option id ="co_two" value="{{ $color->id }}" @if ($outfit->color_id == $color->id) selected @endif>
                                        {{ $color->co_two_value }}
                                    </option>
                                @endif
                            @endforeach
                    ...
                    <button class="return_button" type="submit">更新</button>
                </form>
                <br>
